...
public class InterfazConexionServidor extends JPanel {
	...
	private void conectar() {
		try {
			ServicioCliente.crearInstancia(Integer.parseInt(puerto.getText()),
					host.getText());
			MenuZombieland.setConexcionServidor(true);
			Main.irA(Main.INICIO_SESION);
		} catch (NumberFormatException e) {
			JOptionPane.showMessageDialog(this, "El puerto no es válido",
					"Conexión Zombieland", JOptionPane.WARNING_MESSAGE);
		} catch (ZombielandException e) {
			JOptionPane.showMessageDialog(this, e.getMessage(),
					"Conexión Zombieland", JOptionPane.ERROR_MESSAGE);
		}
	}
	...
	public void paint(Graphics g) {
		ImageIcon imagenFondo = new ImageIcon(
				RutaImagen.get("imagenes/Fondos/fondoServidor.png"));
